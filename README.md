...
- **The preview can diff against what is deployed.** `preview.diff` (opt-in, or `:Chezmoi diff` for one invocation) adds a pane holding the file currently in `$HOME` and puts it in diff mode against the render, so the preview shows what the edit will _change_ out there, not only what it renders to. That is the one comparison git tooling cannot make: the deployed state is in no repository. The deployed side is re-read on every render, so an apply landing underneath shows up immediately.
...
require("chezmoi-template").setup({
  ...
  preview = {
    ...
    diff = false,              -- diff the render against the deployed file in a second pane
  },
